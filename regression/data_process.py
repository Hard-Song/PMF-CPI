...
def generate_Y(dataset_path):
    affinity = open(dataset_path+'/affinity.tsv','r').readlines()
    dict_prot_thisSet = json.load(open(dataset_path + '/proteins.txt'), object_pairs_hook=OrderedDict)
    dict_comp_thisSet = json.load(open(dataset_path + '/compounds.txt'), object_pairs_hook=OrderedDict)
    print("generate Y by dataset:",dataset_path)
    ...
    while same_entry:
        ...
        row = index[0]
        col = index[1]
        y[row][col] = y[row][col]/n
    print('writing to local file...')
    yyy = open(dataset_path+'/Y','wb')
...
dataset = ['toy_reg','BindingDB_reg','AR','JAK','CYP_reg'][int(sys.argv[1])]
